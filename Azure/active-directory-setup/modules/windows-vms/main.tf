...
resource "azurerm_network_interface" "ni02" {
  ...
  ip_configuration {
    ...
    subnet_id                          = var.subnet_id
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id          = azurerm_public_ip.vm_pip02.id
  }
}

## Network Interface Card 03
resource "azurerm_network_interface" "ni03" {
  name                = "${var.vm_name}-ni03"
  location            = var.location
  resource_group_name = var.resource_group_name
  ...
  ip_configuration {
    ...
    subnet_id                          = var.subnet_id
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id          = azurerm_public_ip.vm_pip03.id
  }
}
...
## Associate NIC to Public IP 02
resource "azurerm_network_interface_security_group_association" "ni_nsg02" {
  network_interface_id      = azurerm_network_interface.ni02.id
  network_security_group_id = var.nsg_id01
}

## Associate NIC to Public IP 03
resource "azurerm_network_interface_security_group_association" "ni_nsg03" {
  network_interface_id      = azurerm_network_interface.ni03.id
  network_security_group_id = var.nsg_id01
}
...
## Create Public IP 02 (Windows Client02 - Win11)
resource "azurerm_public_ip" "vm_pip02" {
  name                = "${var.vm_name}-pip02"
  location            = var.location
  resource_group_name = var.resource_group_name
  allocation_method   = "Dynamic"
}

## Create Public IP 03 (Windows Server01 - 2019)
resource "azurerm_public_ip" "vm_pip03" {
  name                = "${var.vm_name}-pip03"
  location            = var.location
  resource_group_name = var.resource_group_name
  allocation_method   = "Dynamic"
}
...
resource "azurerm_windows_virtual_machine" "vm03" {
  name                = "${var.vm_name}_WinServer-03"
  resource_group_name = var.resource_group_name
  location            = var.location
  size                = var.vm_size
  admin_username      = var.admin_username
